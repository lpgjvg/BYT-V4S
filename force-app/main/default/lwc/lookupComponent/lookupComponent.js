...
import fetchRecords from '@salesforce/apex/VolunteerController.fetchRecords';
...
export default class lookupComponent extends LightningElement {
    ...
    fetchSobjectRecords(loadEvent) {
        fetchRecords({
            ...
            if (loadEvent && result && result.length > 0) {
    ...
    handleSelect(event) {
        let selectedRecord = {
            ...
            childCount: event.currentTarget.dataset.childcount
        };
        ...
        this.recId = this.selectedRecordId;
        console.log("Record Id>>>", this.recId);
        console.log("Record Id - Child Count>>>", selectedRecord.childCount);
